"""Six explicit complete-manifest GBDT fits; no test-based model selection."""
...
JOBS = [(year, trees) for year in (2007, 2013, 2016) for trees in (10000, 30000)]
...
def fit_job(year, trees, output, smoke=False):
    ...
    plan = {'identity': receipt['identity'], 'job': name, 'smoke': smoke,
            ...
            'feature_count': 5500, 'normalization': 'StandardScaler fit on all training rows only',
    ...
    meta = {'state': 'complete', 'job': name, 'identity': receipt['identity'], 'smoke': smoke,
            ...
            'trees': settings['n_estimators'], 'feature_count': 5500,
...
def audit_model(target, output, receipt, smoke=False):
    ...
    assert model.n_estimators_ == expected_settings['n_estimators'] and model.n_features_in_ == 5500
    assert model.max_features_ == 74 and model.n_iter_no_change is None
