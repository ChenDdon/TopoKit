...
OUTPUT = PROJECT / 'datasets/protein_ligand_persistent_homology'
EXPECTED = {'train_refined_2007': 1105, 'test_casf_2007': 195,
            'train_refined_2013': 2764, 'test_casf_2013': 195,
...
def atomic_npz(path, values):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile(dir=path.parent, prefix='.', suffix='.npz', delete=False) as f:
        temporary = Path(f.name)
...
def manifests(directory):
    ...
    if len(union) != 4414:
        raise ValueError(f'Expected 4414 unique complexes, got {len(union)}')
    return result, union
...
def prepare(source=SOURCE, output=OUTPUT):
    ...
    for name in ('labels/final', 'features/samples', 'features/records', 'features/failures', 'barcodes/samples',
    ...
    (output / 'README.md').write_text(
        '# Protein–ligand H0 barcode experiment\n\n'
        'Separate 5,500-feature Alpha H0 full-bin barcode experiment. See the workflow README at\n'
        '../../topokit/workflows/protein_ligand_persistent_homology/README.md.\n\n'
        'labels/final contains byte-identical existing 2007/2013/2016 manifests; structures is a\n'
        f'read-only-by-contract symlink to {source}/structures.\n'
        'barcodes contains H0 intervals; features contains full-bin counts and their records;\n'
        'models contains the six fits;\n'
...
def load_feature(sample, output, receipt, expected_inputs=None):
    ...
    if (record['sample'] != sample or record['identity'] != receipt['identity']
            ...
            or record['barcode_sha256'] != sha(output / 'barcodes/samples' / (sample + '.npz'))
...
def load_matrix(values, output, receipt):
    matrix = np.empty((len(values), 5500), dtype=np.float32)
    hashes = {}
