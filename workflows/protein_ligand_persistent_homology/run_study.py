#!/usr/bin/env python3
"""Resumable Cornell experiment: exact features, six fits, independent audit."""
...
import numpy as np
import ph_features as features
import ph_common as common
from ph_train import JOBS, fit_job, audit_model
...
def generate_one(row, output, receipt):
    ...
    try:
        ...
        value, counts, dedup, barcodes, diagnostics = features.compute(selected)
        ...
        barcode_path = output / 'barcodes/samples' / (sample + '.npz')
        common.atomic_npz(barcode_path, features.pack_barcodes(barcodes))
        ...
        record = {'sample': sample, 'identity': receipt['identity'],
                  ...
                  'barcode_sha256': common.sha(barcode_path), 'channel_diagnostics': diagnostics,
...
def audit_features(output, receipt, values):
    ...
    for row in values:
        ...
        if counts.shape != (55, 2) or np.any(counts < 0):
            raise ValueError(f'Invalid channel counts: {sample}')
        if (np.any(value != np.floor(value)) or np.any(value < 0)
                or np.any(np.diff(value, axis=1) > 0) or np.any(value > counts.sum(axis=1)[:, None])
                or np.any(value[-1] != 0)):
            raise ValueError(f'Invalid H0 full-bin counts: {sample}')
        with np.load(output / 'barcodes/samples' / (sample + '.npz'), allow_pickle=False) as archive:
            bars = features.unpack_barcodes(archive)
        # Independent direct interval predicates, not the production range-addition helper.
        for channel, result in enumerate(bars):
            births = np.asarray([b.birth for b in result.intervals])[:, None]
            deaths = np.asarray([b.death for b in result.intervals])[:, None]
            direct = ((births <= features.BIN_EDGES[:-1]) &
                      (deaths >= features.BIN_EDGES[1:])).sum(axis=0)
            np.testing.assert_array_equal(value[channel], direct)
        if (output / 'features/failures' / (sample + '.json')).exists():
            raise ValueError(f'Unresolved failure record: {sample}')
        results.append({'sample': sample, 'feature_sha256': record['feature_sha256'],
                        'barcode_sha256': record['barcode_sha256'], 'inputs': inputs})
    return results
...
def publish(output, receipt, audits):
    ...
    lines = ['# Alpha H0 full-bin barcode results', '', 'All models use 5,500 features and train-only standardization.',
    ...
    lines += ['', 'All six saved models passed independent scaler, reload, membership, prediction and metric audits.',
        ...
        try:
            ...
            if args.action != 'audit':
                common.atomic_json(progress_path, {'stage': 'training', 'utc': common.utc(), 'features_complete': len(values),
                    'models_total': 6, 'models_complete': 0, 'workers': min(6, args.ml_workers), 'smoke': smoke})
                completed = []
                with ProcessPoolExecutor(max_workers=min(6, args.ml_workers), mp_context=mp.get_context('spawn')) as pool:
                    ...
                    for future in as_completed(futures):
                        ...
                        common.atomic_json(progress_path, {'stage': 'training', 'utc': common.utc(),
                            'features_complete': len(values), 'models_total': 6, 'models_complete': len(completed),
            ...
            final = {'state': 'complete', 'utc': common.utc(), 'identity': receipt['identity'], 'smoke': smoke,
                     'feature_count': len(values), 'model_count': 6, 'failures': 0,
            ...
            common.atomic_json(output / ('experiments/smoke/final_audit.json' if smoke else 'experiments/study/final_audit.json'), final)
            if not smoke:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ml-workers', type=int, default=6)
